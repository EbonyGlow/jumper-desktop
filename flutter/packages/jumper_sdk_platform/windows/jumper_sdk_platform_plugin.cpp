...
bool JumperSdkPlatformPlugin::IsTunnelEnabledInLaunchConfig(
  ...
  // Scan for "type" : "tun" pattern in inbounds section.
  // Since we control the config format (generated by SingboxConfigPatcher),
  // a simple string search is sufficient.
  const std::string marker = "\"type\"";
  const std::string tun_value = "\"tun\"";
  size_t pos = 0;
  while ((pos = content.find(marker, pos)) != std::string::npos) {
    ...
    if (val != std::string::npos &&
        content.compare(val, tun_value.size(), tun_value) == 0) {
      return true;
    }
  }
  return false;
